...
export interface RealtimeConfig {
    ...
    perceptionConfig: {
        ...
        internalizationBias: number;
        frequencyPenalty: number;
        presencePenalty: number;
        batteryUrgencyThreshold: number;
...
export const defaultRealtimeConfig: RealtimeConfig = {
    ...
    perceptionConfig: {
        ...
        internalizationBias: 0.8,      // 默认高内化，倾向于通过语气体现
        frequencyPenalty: 0.5,         // 重复惩罚：提过一次后大幅降低权重
        presencePenalty: 0.3,          // 存在惩罚：提过之后小幅干扰
        batteryUrgencyThreshold: 15,    // 15% 以下算紧急
        lateNightHour: 23              // 11点以后算深夜
    }
};
...
export const RealtimeContextManager = {
    ...
    fetchTavilyNews: async (apiKey: string): Promise<NewsItem[]> => {
        try {
            const response = await SearchTool.search('中国 热点新闻 今日', {
                engine: 'tavily',
                apiKey,
                maxResults: 5,
                searchDepth: 'basic',
    ...
    fetchNews: async (config: RealtimeConfig): Promise<NewsItem[]> => {
        ...
        if (config.newsApiKey) {
            const provider = config.newsProvider || 'brave';
            news = provider === 'tavily'
                ? await RealtimeContextManager.fetchTavilyNews(config.newsApiKey)
                : await RealtimeContextManager.fetchBraveNews(config.newsApiKey);
            if (news.length > 0) {
     ...
     */
    performSearch: async (query: string, apiKey: string, provider: 'brave' | 'tavily' = 'brave'): Promise<{ success: boolean; results: SearchResult[]; message: string }> => {
        if (!query || !apiKey) {
        ...
        try {
            if (provider === 'tavily') {
                const data = await SearchTool.search(query, {
                    engine: 'tavily',
                    apiKey,
                    maxResults: 5,
                ...
                return results.length > 0
                    ? { success: true, results, message: '搜索成功' }
                    : { success: false, results: [], message: '没有找到相关结果' };
            }
