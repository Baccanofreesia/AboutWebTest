...
const Appearance: React.FC = () => {
    ...
    const THEME_PRESETS: { name: string, config: Partial<OSTheme>, color: string }[] = [
        { name: '雾霾蓝\n(Fog Blue)', config: { hue: 215, saturation: 25, lightness: 65, contentColor: '#ffffff' }, color: 'hsl(215, 25%, 65%)' },
        { name: '灰紫兰\n(Dusty Violet)', config: { hue: 260, saturation: 20, lightness: 70, contentColor: '#ffffff' }, color: 'hsl(260, 20%, 70%)' },
        { name: '豆沙绿\n(Sage Green)', config: { hue: 140, saturation: 15, lightness: 65, contentColor: '#ffffff' }, color: 'hsl(140, 15%, 65%)' },
        { name: '燕麦色\n(Oatmeal)', config: { hue: 35, saturation: 30, lightness: 80, contentColor: '#64748b' }, color: 'hsl(35, 30%, 80%)' },
        { name: '干枯玫瑰\n(Dried Rose)', config: { hue: 350, saturation: 25, lightness: 70, contentColor: '#ffffff' }, color: 'hsl(350, 25%, 70%)' },
    ];
                    ...
                    <>
                        ...
                        <section className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100">
                            ...
                            <div className="aspect-[9/16] w-1/2 mx-auto bg-slate-100 rounded-2xl overflow-hidden relative shadow-inner mb-4 group cursor-pointer" onClick={() => wallpaperInputRef.current?.click()}>
                                {theme.wallpaper.startsWith('http') || theme.wallpaper.startsWith('data:') ? (
                                    <img src={theme.wallpaper} className="w-full h-full object-cover" />
                                ) : (
                                    <div className="w-full h-full" style={{ background: theme.wallpaper }} />
                                )}
                                ...
                                <div key={app.id} className="flex flex-col items-center gap-2">
                                    ...
                                    >
                                        {customUrl ? (
                                            ...
                                            <div className={`w-full h-full ${app.color} flex items-center justify-center text-white text-lg font-bold`}>
                                                {app.name.charAt(0)}
                                            </div>
                                        )}
